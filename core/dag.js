...
async function ValidUnit(unit, dag_root, log_limit, chain, db) {
    await db.open();
...
async function Unit_to_Dag(unit, dag_root, db) {
    await db.open();
    const DagData = new merkle_patricia_1.Trie(db, dag_root);
...
async function Unit_to_Memory(unit, memory_root, db) {
    await db.open();
    const Memory = new merkle_patricia_1.Trie(db, memory_root);
    let target = await Memory.get(unit.contents.data.request);
...
async function AcceptUnit(unit, dag_root, memory_root, log_limit, chain, db) {
    if (!await ValidUnit(unit, dag_root, log_limit, chain, db))
        return [dag_root, memory_root];
    console.log("OK");
...
async function GetEdgeDag(root, db) {
    await db.open();
    const DagData = new merkle_patricia_1.Trie(db, root);
...
async function CreateUnit(password, pub_key, request, index, payee, dag_root, difficulty, log, db) {
