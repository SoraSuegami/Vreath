...
exports.block_accept = async (block, chain, candidates, roots, pool, not_refreshed, now_buying, unit_store) => {
    ...
    if (accepted.block.length > 0 && !reqested) {
        ...
        script_1.store.commit("refresh_unit_store", _.copy(new_unit_store));
        return {
...
exports.check_chain = async (new_chain, my_chain, pool, codes, secret, unit_store) => {
    if (new_chain.length > my_chain.length) {
        ...
        script_1.store.commit('refresh_yet_data', _.copy(add_blocks_data).concat(script_1.store.state.yet_data));
...
exports.unit_buying = async (secret, units, roots, chain) => {
    try {
        ...
        const pure_native_tx = TxSet.CreateRequestTx(pub_key, native_remiter, Math.pow(2, -3), "issue", con_1.native, _.copy([native_remiter].concat(unit_sellers)), ["remit", JSON.stringify(prices)], [], con_1.my_version, TxSet.empty_tx_pure().meta.pre, TxSet.empty_tx_pure().meta.next, Math.pow(2, -18));
