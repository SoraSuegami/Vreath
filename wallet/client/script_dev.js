...
const store = new vuex_1.default.Store({
    ...
    actions: {
        async read({ state, commit }) {
            ...
            const balance = await db_1.get('balance', state.balance);
            commit('refresh_secret', secret);
...
const Best_lang = {
    ...
    computed: {
        bar_data: function () {
            const names = this.langs.map(lang => lang.name);
    ...
    created: async function () {
        ...
        worker.addEventListener('message', (event) => {
            ...
            if (index != -1) {
                this.balances = _.new_obj(this.balances, (bs) => {
                    bs[index] = new bignumber_js_1.default(event.data.amount).times(100).toNumber() || 0;
                    return bs;
                });
            }
        });
